...
import type { UpdateProjectScheduleInput } from '../../types/project-review-organization';

type ProjectScheduleForm = {
  meetingUrl: string;
  reviewLocation: string;
  reviewTime: string;
};

type ProjectSchedulePanelProps = {
...
export function ProjectSchedulePanel({
  ...
  const [form, setForm] = useState<ProjectScheduleForm>({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const input: UpdateProjectScheduleInput = {
      meetingUrl: form.meetingUrl.trim(),
      reviewLocation: form.reviewLocation.trim(),
    };

    if (form.reviewTime) {
      input.reviewTime = new Date(form.reviewTime).toISOString();
    }
    ...
    try {
      const updated = await updateProjectSchedule(project.id, input);
      setNotice('已保存评审安排。');
      onSaved(updated);
    } catch (submitError) {
    ...
    <section className="panel">
      <div className="panel-body">
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="m-0 text-lg font-bold text-slate-950">评审安排</h2>
            <p className="mt-1 text-sm leading-6 text-slate-500">
              当前仅保存会议链接字段，不做腾讯会议直播或 API 集成。
            </p>
          </div>
          {project.meetingUrl ? (
            <a
