const FALLBACK_FILENAME = 'uploaded.xlsx';

const CJK_PATTERN =
  /[\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff\u3000-\u303f\uff00-\uffef]/;
const REPLACEMENT_PATTERN = /\uFFFD/g;
const MOJIBAKE_PATTERN =
  /(?:Ã|Â|Ä|Å|Æ|Ç|ä|å|æ|ç|è|é|ï¼|ï½|•|ˆ|„|œ|›|‰|\uFFFD)/g;
...
export function normalizeUploadedFilename(originalname: string): string {
  ...
  if (!hasMojibakeSignal(trimmed)) {
    return trimmed;
  }

  const candidate = decodeMojibakeCandidate(trimmed);

  return shouldUseDecodedCandidate(trimmed, candidate) ? candidate : trimmed;
}
