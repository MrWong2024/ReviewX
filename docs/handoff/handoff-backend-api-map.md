...
| project-imports | `GET` | `/admin/project-imports` | `ProjectImportsController` | `ProjectImportsService` | `SessionAuthGuard` + `RolesGuard(admin)` | `QueryProjectImportJobsDto` | `{ items, page, pageSize, total }` | implemented | 任务列表分页，`pageSize` 最大 `1000`；支持 `status/batchId/keyword`；历史任务可继续按全部批次筛选，包括已停用批次 |
| project-imports | `GET` | `/admin/project-imports/:id` | `ProjectImportsController` | `ProjectImportsService` | `SessionAuthGuard` + `RolesGuard(admin)` | path `id` | `ProjectImportJobResponse` | implemented | 返回任务基本信息、字段映射快照和统计计数，不内联全部 rows；不因关联批次停用而拒绝查看 |
...
| project-imports | `POST` | `/admin/project-imports/:id/rows/:rowId/confirm` | `ProjectImportsController` | `ProjectImportsService` | `SessionAuthGuard` + `RolesGuard(admin)` | 无 | `ProjectImportRowResponse` | implemented | 仅 `importable` 可确认；创建或更新 Project，记录 `projectId/confirmedByUserId/confirmedAt`；已存在导入任务不因批次停用改变确认口径 |
...
| project-imports | `POST` | `/admin/project-imports/:id/rows/:rowId/skip` | `ProjectImportsController` | `ProjectImportsService` | `SessionAuthGuard` + `RolesGuard(admin)` | 无 | `ProjectImportRowResponse` | implemented | `importable/pending_confirmation/failed` 可跳过；`confirmed` 返回 `409`；已存在导入任务不因批次停用改变跳过口径 |
...
| expert-reviews | `GET` | `/expert/review-tasks` | `ExpertReviewTasksController` | `ExpertReviewsService` | `SessionAuthGuard` + `RolesGuard(expert)` | `QueryExpertReviewTasksDto` | `{ items, page, pageSize, total }`；`items[].project.reviewManager` 为 `{ id, name, phone? }` 或 `null` | implemented | 只返回当前专家 `status=assigned` 的启用项目；支持 `batchId/keyword/status/reviewManagerId/reviewSchemeId`；无评分显示 `not_started`；缺评审方案项目仍返回，`project.reviewSchemeId=null` 供前端禁用评分入口；缺评审时间项目仍返回，`project.reviewTime=null` 供前端显示“暂不能评分”；列表批量查询当前页项目评审负责人摘要，不返回敏感字段 |
| expert-reviews | `GET` | `/expert/review-tasks/:projectId` | `ExpertReviewTasksController` | `ExpertReviewsService` | `SessionAuthGuard` + `RolesGuard(expert)` | path `projectId` | `{ project, materialCount, materials, reviewSchemeSnapshot, review }`；`project.reviewManager` 为 `{ id, name, phone? }` 或 `null` | implemented | 当前专家必须已分配；项目无 `reviewSchemeSnapshot` 返回 `409 PROJECT_REVIEW_SCHEME_MISSING`，message 为“项目尚未分配评审方案，暂不能评分。”；项目无 `reviewTime` 返回 `409 PROJECT_REVIEW_TIME_MISSING`，message 为“项目尚未设置评审时间，暂不能评分。”；无评分时返回空评分结构；按当前项目 `reviewManagerId` 内联负责人摘要，支持 admin + review_manager 多角色用户 |
| expert-reviews | `PUT` | `/expert/review-tasks/:projectId` | `ExpertReviewTasksController` | `ExpertReviewsService` | `SessionAuthGuard` + `RolesGuard(expert)` | `SaveExpertReviewDto` | `ExpertReviewResponse` | implemented | 保存草稿；仅 `draft/returned/not_started` 可写；项目无 `reviewSchemeSnapshot` 返回 `409 PROJECT_REVIEW_SCHEME_MISSING` 和“项目尚未分配评审方案，暂不能评分。”；项目无 `reviewTime` 返回 `409 PROJECT_REVIEW_TIME_MISSING` 和“项目尚未设置评审时间，暂不能评分。”；`Project.reviewTime` 已设置但未到时仍可保存草稿；`submitted` 返回 `409` 且不可修改口径不变；`totalScore` 后端计算 |
...
| expert-reviews | `POST` | `/expert/review-tasks/:projectId/submit` | `ExpertReviewTasksController` | `ExpertReviewsService` | `SessionAuthGuard` + `RolesGuard(expert)` | `SaveExpertReviewDto` | `ExpertReviewResponse` | implemented | 提交最终评分；项目无 `reviewSchemeSnapshot` 返回 `409 PROJECT_REVIEW_SCHEME_MISSING` 和“项目尚未分配评审方案，暂不能评分。”；项目无 `reviewTime` 返回 `409 PROJECT_REVIEW_TIME_MISSING` 和“项目尚未设置评审时间，暂不能评分。”；若 `Project.reviewTime` 存在且服务器当前时间早于评审时间，返回 `409 REVIEW_NOT_STARTED` 且不写 `submittedAt`；完整校验 score、评价描述、改进建议条件必填；重复 submitted 返回 `409` 且不可修改口径不变 |
...
| expert-reviews | `POST` | `/review-manager/projects/:id/expert-reviews/:expertUserId/return` | `ReviewManagerExpertReviewsController` | `ExpertReviewsService` | `SessionAuthGuard` + `RolesGuard(review_manager)` | `ReturnExpertReviewDto` | `ExpertReviewResponse` | implemented | 必须是当前评审负责人负责项目；仅 `submitted` 且项目无 `confirmed` 合议时可退回；已有 confirmed 合议返回 `409` 且不写退回状态；退回成功写 `returnedAt/returnedByUserId/returnReason`；专家可重新编辑提交 |
...
- 短信登录时只允许已有 active 用户；验证码校验调用 `CheckSmsVerifyCode`，仅 `Success === true`、`Code === 'OK'`、`Model.VerifyResult === 'PASS'` 同时满足才视为通过。
...
- 门户参考数据 `/portal/reference-data/*`：要求 `SessionAuthGuard` 登录，允许 `project_owner/expert/review_manager/client/admin` 读取；无角色或非允许角色返回 `403`；`users` 摘要接口禁止查询 `admin` 角色，查询 `review_manager/expert/project_owner` 时会返回兼任 `admin` 的该业务角色用户，纯 `admin` 不会命中
