...
- 项目导入上传批次下拉只显示启用批次；历史任务筛选批次下拉仍显示全部批次，包括已停用批次。
- 项目导入无启用批次时上传控件应禁用并提示“暂无启用批次，请先启用或创建批次后再导入项目。”；提交前必须校验所选批次仍为启用状态。
- 后端返回 `BATCH_INACTIVE_FOR_PROJECT_IMPORT` 或“批次已停用，不能导入项目。”时，项目导入上传区应展示该业务 message；停用批次下的历史导入任务仍可查看，已存在任务的确认 / 跳过流程不因批次停用而隐藏。
- 项目评审组织可设置评审负责人、评审方案、评审时间、地点和 `meetingUrl`。
- `/admin/projects` 操作列提供“编辑”入口，可打开项目基础信息编辑弹窗。
- 项目基础信息编辑应回显并保存项目编号、名称、批次、项目类型、项目状态、项目负责人、承担单位、合作单位、拨款总额、已拨款、学科、受理处室和启用状态，保存成功后刷新当前列表。
- 项目基础信息编辑不提交评审负责人、评审方案、评审时间地点、会议链接、最终等级、导入来源和评审方案快照；保存基础信息不应清空既有评审负责人 / 评审方案或评审安排。
- 项目基础信息编辑遇到同批次项目编号重复、无效负责人 / 单位 / 字典等后端拒绝时，应在弹窗内展示后端错误并保持弹窗打开。
- `/admin/projects` 宽表格中“组织状态”和“操作”列固定在右侧，横向滚动时仍可见。
...
- 已分配且有评审方案但缺评审时间的项目仍显示在 `/expert/review-tasks`，不隐藏；列表不显示可点击“开始评分”，改为“暂不能评分”并说明“项目尚未设置评审时间”。
- 直接访问缺评审时间项目详情，或保存/提交该项目评分时，后端 `409 PROJECT_REVIEW_TIME_MISSING` 应展示“项目尚未设置评审时间，暂不能评分。”，不得误显示“评分已提交，不能修改”；处理口径与 `PROJECT_REVIEW_SCHEME_MISSING` 保持一致。
