...
- 忘记密码 / 短信验证码找回密码前端待接入；后端已提供 `POST /auth/password-reset/code` 和 `POST /auth/password-reset`。
- 找回密码发送验证码应展示后端通用提示，不展示验证码、RequestId、token、Cookie 或账号存在性信息。
- 找回密码重置成功后不自动进入登录态；后端不写 Set-Cookie、不返回用户或 token，前端应提示用户使用新密码重新登录。
...
- 文件预览 / 在线转换暂缓。
- 审计日志暂缓。
- 忘记密码 / 短信验证码找回密码前端接入、批量导入增强、权限矩阵配置仍暂缓；短信验证码登录前后端已完成，短信验证码找回密码后端已完成且前端待接入，自助改密已完成，不再作为后端暂缓项；短信注册仍未实现。
...
- 修改腾讯会议、AI 配置管理 / 调用日志 / 流式输出、文件预览 / 在线转换、审计日志、批量导入增强、权限矩阵、忘记密码 / 短信验证码找回密码前端接入或短信验证码登录前端切换时，先更新本手册再执行对应专项验证。
