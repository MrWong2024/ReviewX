...
- 当前短哈希基线：`b6356b2`；演示版收口与生产预检已完成，服务器试跑和甲方演示基本就绪。
...
- 认证与用户：手机号密码登录、HttpOnly Cookie Session、`/auth/me`、当前用户自助修改本人密码、管理员用户 CRUD、角色 / 启用状态 / 单位 / 学科关联、默认手机号密码和最后启用 admin 保护已完成。
...
- 腾讯会议监管前端、文件预览、忘记密码 / 短信验证码、用户批量导入、专家库批量导入、权限矩阵配置仍未实现；AI 配置管理页面、AI 调用日志和流式输出仍未实现。

### 5.2 后端未完成

- 腾讯会议直播 / 推流 / 回看 / 会议 API 集成、文件预览 / 在线转换、忘记密码 / 短信验证码、用户批量导入、专家库批量导入、操作审计日志、更细粒度权限矩阵、AI 配置管理和 AI 调用日志仍未实现。
...
6. 后续按实际需求进入部署维护、小范围演示打磨或暂缓能力专项开发：腾讯会议直播 / 推流 / 回看 / 会议 API 集成、文件预览 / 在线转换、审计日志、忘记密码 / 短信验证码、批量导入增强、权限矩阵配置、AI 配置管理和 AI 调用日志仍暂缓；AI 辅助生成合议草稿已接入，最终确认仍人工复核。
...
- 忘记密码、短信验证码、用户批量导入、专家库批量导入和权限矩阵配置暂缓；用户自助改密已完成，不再作为暂缓项。
