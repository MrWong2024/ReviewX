...
本路线图描述的是截至“ReviewX 前端第五阶段：专家工作台与评分”完成，并完成“ReviewX 小修：AdminShell 增加返回工作台入口”“ReviewX 小修：普通字典类型筛选固定化并默认选中项目状态”“ReviewX 小修：普通字典固定类型恢复评审等级”“ReviewX 小修：树形字典列表行内移除重复树类型展示”“ReviewX 小修：树形字典默认仅展示第一层并支持按需展开”和“ReviewX 小修：树形字典新增编辑弹窗父节点选择体验优化”后的项目状态。
...
- 管理员树形字典页顶部树类型筛选继续控制当前维护范围，列表默认仅展示第一层节点，支持按需逐层展开 / 收起；列表行内仅保留编码、排序和全称 / 路径等节点信息，不再逐行重复显示树类型；新增 / 编辑弹窗父节点下拉默认仅列一级节点，可勾选显示全部层级。
...
- 专家评分提交前已有基础校验和二次确认；后端仍是最终校验与权限边界，submitted 状态前端只读，returned 状态可保存草稿并重新提交。
