...
- 当前短哈希基线：`658cdc4`；演示版收口与生产预检已完成，服务器试跑和甲方演示基本就绪。
...
- 已完成短信验证码登录后端能力：新增 `POST /auth/sms-login/code` 和 `POST /auth/sms-login`，接入阿里云号码认证 `SendSmsVerifyCode` / `CheckSmsVerifyCode`，验证码由阿里云生成和校验，成功后复用现有服务端 Session + HttpOnly Cookie；前端登录页密码 / 验证码自选仍待接入。
...
- 认证与用户：手机号密码登录、短信验证码登录后端、HttpOnly Cookie Session、`/auth/me`、当前用户自助修改本人密码、管理员用户 CRUD、角色 / 启用状态 / 单位 / 学科关联、默认手机号密码和最后启用 admin 保护已完成。
...
- 公共工作台 / 登录态：Next.js App Router + TypeScript + Tailwind CSS 4，统一 fetch API Client、HttpOnly Cookie 会话协作、`/workspace` 多角色入口、`/account/change-password` 自助修改密码页和五类业务 Shell 已完成。
...
- 腾讯会议监管前端、文件预览、短信验证码登录前端切换、忘记密码 / 短信验证码找回密码、用户批量导入、专家库批量导入、权限矩阵配置仍未实现；AI 配置管理页面、AI 调用日志和流式输出仍未实现。

### 5.2 后端未完成

- 腾讯会议直播 / 推流 / 回看 / 会议 API 集成、文件预览 / 在线转换、忘记密码 / 短信验证码找回密码、用户批量导入、专家库批量导入、操作审计日志、更细粒度权限矩阵、AI 配置管理和 AI 调用日志仍未实现。
...
6. 后续按实际需求进入部署维护、小范围演示打磨或暂缓能力专项开发：短信验证码登录前端切换待接入；腾讯会议直播 / 推流 / 回看 / 会议 API 集成、文件预览 / 在线转换、审计日志、忘记密码 / 短信验证码找回密码、批量导入增强、权限矩阵配置、AI 配置管理和 AI 调用日志仍暂缓；AI 辅助生成合议草稿已接入，最终确认仍人工复核。
...
- 短信验证码登录后端已完成，下一阶段可只接前端登录页“密码登录 / 验证码登录”切换和接口联调；忘记密码、验证码找回密码和短信注册仍不在当前能力内。
...
- 忘记密码 / 短信验证码找回密码、用户批量导入、专家库批量导入和权限矩阵配置暂缓；短信验证码登录后端已完成但前端待接入，用户自助改密已完成，不再作为暂缓项。
...
- 短信验证码登录生产依赖 `SMS_AUTH_PROVIDER=aliyun` 与 `ALIYUN_SMS_*` 配置；自动化测试固定走 stub，不调用真实阿里云短信 API，不保存或返回验证码。
