...
- 认证与用户：手机号密码登录、短信验证码登录、HttpOnly Cookie Session、`/auth/me`、当前用户自助修改本人密码、管理员用户 CRUD、角色 / 启用状态 / 单位 / 学科关联、默认手机号密码和最后启用 admin 保护已完成。
...
- 腾讯会议监管前端、文件预览、忘记密码 / 短信验证码找回密码、用户批量导入、专家库批量导入、权限矩阵配置仍未实现；AI 配置管理页面、AI 调用日志和流式输出仍未实现。
...
6. 后续按实际需求进入部署维护、小范围演示打磨或暂缓能力专项开发：短信验证码登录前后端已完成；腾讯会议直播 / 推流 / 回看 / 会议 API 集成、文件预览 / 在线转换、审计日志、忘记密码 / 短信验证码找回密码、批量导入增强、权限矩阵配置、AI 配置管理和 AI 调用日志仍暂缓；AI 辅助生成合议草稿已接入，最终确认仍人工复核。
...
- 短信验证码登录前后端已完成；忘记密码、验证码找回密码和短信注册仍不在当前能力内。
...
- 忘记密码 / 短信验证码找回密码、用户批量导入、专家库批量导入和权限矩阵配置暂缓；短信验证码登录前后端已完成，用户自助改密已完成，不再作为暂缓项。
