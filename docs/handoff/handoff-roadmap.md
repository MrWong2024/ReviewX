...
- 当前短哈希基线：`452bf1b`；演示版收口与生产预检已完成，服务器试跑和甲方演示基本就绪。
...
- 已完成短信验证码找回密码后端能力：后端提供 `POST /auth/password-reset/code` 和 `POST /auth/password-reset`，复用阿里云短信认证服务；找回密码成功后不自动登录、不写 Set-Cookie、不返回用户或 token，并清理该用户已有 sessions；前端忘记密码页待接入。
...
- 认证与用户：手机号密码登录、短信验证码登录、短信验证码找回密码后端、HttpOnly Cookie Session、`/auth/me`、当前用户自助修改本人密码、管理员用户 CRUD、角色 / 启用状态 / 单位 / 学科关联、默认手机号密码和最后启用 admin 保护已完成。
...
- 公共工作台 / 登录态：Next.js App Router + TypeScript + Tailwind CSS 4，统一 fetch API Client、HttpOnly Cookie 会话协作、`/login` 密码登录 / 短信验证码登录自选、`/workspace` 多角色入口、`/account/change-password` 自助修改密码页和五类业务 Shell 已完成。
...
- 腾讯会议监管前端、文件预览、忘记密码 / 短信验证码找回密码前端接入、用户批量导入、专家库批量导入、权限矩阵配置仍未实现；AI 配置管理页面、AI 调用日志和流式输出仍未实现。

### 5.2 后端未完成

- 腾讯会议直播 / 推流 / 回看 / 会议 API 集成、文件预览 / 在线转换、短信注册、用户批量导入、专家库批量导入、操作审计日志、更细粒度权限矩阵、AI 配置管理和 AI 调用日志仍未实现；短信验证码找回密码后端已完成，前端待接入。
...
6. 后续按实际需求进入部署维护、小范围演示打磨或暂缓能力专项开发：短信验证码登录前后端已完成；短信验证码找回密码后端已完成、前端待接入；腾讯会议直播 / 推流 / 回看 / 会议 API 集成、文件预览 / 在线转换、审计日志、批量导入增强、权限矩阵配置、AI 配置管理和 AI 调用日志仍暂缓；AI 辅助生成合议草稿已接入，最终确认仍人工复核。
...
- 短信验证码登录前后端已完成；短信验证码找回密码后端已完成，忘记密码 / 验证码找回密码前端页待接入；短信注册仍未实现。
...
- 忘记密码 / 短信验证码找回密码前端接入、用户批量导入、专家库批量导入和权限矩阵配置暂缓；短信验证码登录前后端已完成，短信验证码找回密码后端已完成，用户自助改密已完成，不再作为后端暂缓项；短信注册仍未实现。
