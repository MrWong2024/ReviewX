...
本路线图描述的是截至“ReviewX 第四阶段补丁一：门户端只读基础数据接口”完成后的项目状态。
...
- 项目负责人完整材料上传闭环：
  ...
  - 后端已补齐 project_owner 可访问的 `material_type` 字典读取接口和项目展示名称映射只读接口
  - 材料上传真实提交入口仍需前端在下一补丁中接入门户参考数据并启用
  - 项目基础信息名称映射仍需前端在下一补丁中接入门户参考数据，替换 ID 兜底展示
- 专家工作台：
  - 评审任务列表
  - 项目详情
  - 材料查看
...
5. ReviewX 前端第四阶段：项目负责人工作台与材料管理（前端接入已完成，上传真实提交待下一补丁启用）
   - 项目负责人项目列表 / 详情
   - 后续推进需求
   - 材料上传入口和前端校验
   - 材料列表
   - 下载 URL
   - 软删除
   - 后端门户参考数据契约已补齐，待前端接入启用材料上传和名称映射
...
- 当前建议下一步是“ReviewX 第四阶段补丁二：项目负责人材料上传闭环启用与名称映射优化”。
- 后端门户参考数据契约已补齐，下一补丁应回到项目负责人详情页启用材料类型选择并复测材料上传真实提交闭环，同时接入批次、项目状态、项目类型、学科、单位、评审负责人和评审方案名称映射。
- 再下一建议前端阶段仍是“ReviewX 前端第五阶段：专家工作台与评分”。
- 字段映射配置前端已补齐项目导入字段映射配置闭环。
- 管理员项目评审组织前端已接入评审负责人/方案分配、评审安排、专家候选、专家追加/替换/移除和批量设置专家；专家候选和分配规则仍以后端校验为准。
- 项目负责人工作台已接入项目列表、详情、后续推进需求、材料列表、下载和软删除；后端已补齐门户参考数据，材料上传入口仍待前端下一补丁启用。
...
- 当前门户参考数据接口已补齐 project_owner 可访问的 `material_type`、项目状态、树形字典、批次、单位、评审负责人和评审方案摘要；前端仍需在下一补丁接入，当前项目负责人页面可能继续按既有实现禁用上传或 ID 兜底展示。
