...
- 后端第八阶段甲方看板统计 API 已完成：已提供 `client` 角色专用 `/client/dashboard/overview` 与 `/client/dashboard/projects`；甲方看板前端尚未实现，不得写成已完成。
...
3. 当前建议下一阶段：ReviewX 前端第九阶段：甲方看板。
...
- 优先做“甲方看板前端”，接入后端已完成的 `/client/dashboard/overview` 与 `/client/dashboard/projects`，实现统计卡片、图表、项目列表钻取和评审现场 `meetingUrl` 入口。
- 如果演示优先级更高，可先做申诉、材料、专家分配锁定、合议链路和甲方看板统计 API 的小范围联调回归；不得把甲方看板前端误标为已完成。
