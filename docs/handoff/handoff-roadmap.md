...
- 专家评分提交前已有基础校验、评审时间窗口提示和二次确认；后端仍是最终校验与权限边界，项目 `reviewTime` 未到时返回 `409 REVIEW_NOT_STARTED`，submitted 状态前端只读，returned 状态可保存草稿并在评审开始后重新提交。
