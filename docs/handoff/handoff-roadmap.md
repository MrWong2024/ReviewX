...
本路线图描述的是截至“ReviewX 第四阶段补丁三：项目材料草稿提交状态与物理删除规则后端化”完成后的项目状态。
...
- 项目负责人材料提交与删除规则前端接入：
  - “提交评审材料”按钮
  - `draft/submitted/legacy active` 状态展示
  - submitted 材料删除入口禁用和 `409` 错误提示
- admin 材料删除入口和 reason 表单
- 专家工作台：
  - 评审任务列表
  - 项目详情
  - 材料查看
...
9. ReviewX 第四阶段补丁四：项目负责人材料提交与删除规则前端接入
   - 项目负责人详情页增加“提交评审材料”入口
   - 展示 `draft/submitted/legacy active` 状态
   - 对 submitted 材料禁用/隐藏项目负责人删除入口
   - 对后端 `409` 做清晰错误提示
   - admin 删除入口如纳入前端，必须填写 reason

10. ReviewX 前端第五阶段：专家工作台与评分
   - 专家任务列表
   - 项目和材料查看
   - 评分草稿
   - 条件必填建议
...
- 当前建议下一步是“ReviewX 第四阶段补丁四：项目负责人材料提交与删除规则前端接入”。
- 再下一建议阶段是“ReviewX 前端第五阶段：专家工作台与评分”。
...
- 项目负责人工作台已接入项目列表、详情、后续推进需求、门户参考数据名称映射、材料列表、上传、下载和删除入口；`material_type` 为空或 reference-data 加载失败时上传入口禁用。
- 后端当前已将新上传项目材料默认设为 `draft`，并提供 `POST /project-owner/projects/:id/materials/submit`；前端尚未接入提交按钮，提交前评审负责人 / 专家看不到新上传 draft 材料。
- 后端当前已将项目负责人 DELETE 材料语义改为物理删除 `draft/legacy active`，`submitted` 返回 `409`；当前前端若误删 submitted 会显示错误，需补丁四接入状态展示和禁用逻辑。
- admin 删除材料后端接口已实现且要求 reason；当前前端未接入 admin 删除入口。
...
- 当前项目材料状态机已后端化，但前端尚未展示 `draft/submitted` 状态，也没有“提交评审材料”按钮；进入专家工作台前应优先完成补丁四。
