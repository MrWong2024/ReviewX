...
- 申诉闭环前端接入：
  ...
  - 项目负责人发起申诉前端按后端规则做体验层限制：必须已有 confirmed 合议和 finalLevel、最多 3 次、存在 submitted / processing 申诉时禁用再次提交
...
- 申诉闭环前端已接入项目负责人、评审负责人和管理员三类页面；项目负责人可查看合议并按规则提交申诉和附件，评审负责人 / 管理员可下载附件并处理 accepted / rejected，等级变更历史以后端 `level-history` 为准。
