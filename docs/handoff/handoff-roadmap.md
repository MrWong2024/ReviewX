...
本路线图描述的是截至“ReviewX 第四阶段补丁二：项目负责人材料上传闭环启用与名称映射优化”完成后的项目状态。
...
- 项目负责人工作台前端接入：
  ...
  - 项目详情和评审安排名称展示
  - 后续推进需求填写 / 保存
  - 材料列表、按 `material_type` 筛选、下载 URL 和软删除
  - 文件数量、大小和扩展名校验逻辑已实现
...
- 项目负责人工作台已接入项目列表、详情、后续推进需求、门户参考数据名称映射、材料列表、上传、下载和软删除；`material_type` 为空或 reference-data 加载失败时上传入口禁用。
