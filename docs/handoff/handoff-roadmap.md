...
- 本文档不替代 backend / frontend handoff 细节：后端接口见 `handoff-backend-api-map.md`，DTO 字段见 `handoff-backend-dto-cheatsheet.md`，Service 职责见 `handoff-backend-service-map.md`，前端接口调用见 `handoff-frontend-api-map.md`，路由细节见 `handoff-frontend-route-map.md`，组件职责见 `handoff-frontend-component-map.md`，验证口径见 `handoff-frontend-testing-playbook.md`。

## 2. 当前承接状态

- 当前短哈希基线：`4715298`；演示版收口与生产预检已完成，服务器试跑和甲方演示基本就绪。
...
- 已同步合议草稿 AI 小修：评审负责人生成合议草稿优先调用百炼 OpenAI-compatible Chat Completions，配置来自 `LLM_PROVIDER/BAILIAN_*`；AI 不可用、超时、HTTP 失败或响应不可解析时 fallback 到 `rule_based` 规则汇总；生成草稿后仍不自动填入最终确认表单，最终合议仍由人工确认；同项目生成草稿默认 60 秒冷却。
...
- 已完成用户自助修改密码：登录用户可通过 `PATCH /auth/me/password` 和前端 `/account/change-password` 修改本人密码，成功后 `mustChangePassword=false`，当前会话保持有效，不返回密码哈希或 session token。
- 已完成短信验证码登录前后端能力：后端提供 `POST /auth/sms-login/code` 和 `POST /auth/sms-login`，成功后复用现有服务端 Session + HttpOnly Cookie；前端 `/login` 已提供密码登录 / 验证码登录自选、验证码发送和 60 秒倒计时。
- 已完成短信验证码找回密码前后端能力：后端提供 `POST /auth/password-reset/code` 和 `POST /auth/password-reset`，复用阿里云短信认证服务；前端 `/forgot-password` 已接入手机号、短信验证码、新密码和确认密码表单；找回密码成功后不自动登录、不写 Set-Cookie、不返回用户或 token，并清理该用户已有 sessions。
...
- 认证与用户：手机号密码登录、短信验证码登录、短信验证码找回密码后端、HttpOnly Cookie Session、`/auth/me`、当前用户自助修改本人密码、管理员用户 CRUD、角色 / 启用状态 / 单位 / 学科关联、默认手机号密码和最后启用 admin 保护已完成；短信注册仍未实现。
...
- 专家评分与合议：专家任务、材料查看、草稿 / 提交、删除本人 draft 草稿、评审时间提交限制、退回重提、评分汇总、AI 辅助合议草稿、`rule_based` fallback、人工确认最终意见 / 分数 / 等级已完成；confirmed 合议不可在合议页再次覆盖。
...
- 公共工作台 / 登录态：Next.js App Router + TypeScript + Tailwind CSS 4，统一 fetch API Client、HttpOnly Cookie 会话协作、`/login` 密码登录 / 短信验证码登录自选、`/forgot-password` 短信验证码找回密码、`/workspace` 多角色入口、`/account/change-password` 自助修改密码页和五类业务 Shell 已完成。
...
- 评审负责人端：负责项目列表、项目总览、评审组织、专家分配、材料只读、专家评分查看 / 退回、评分汇总、AI 辅助合议草稿 / 规则 fallback / 生成冷却 / 人工确认、项目申诉查看 / 处理已接入；review-manager 命名空间只看自己负责项目，不混用 admin 全局能力。
...
- 腾讯会议监管前端、文件预览、用户批量导入、专家库批量导入、权限矩阵配置仍未实现；AI 配置管理页面、AI 调用日志和流式输出仍未实现。

### 5.2 后端未完成

- 腾讯会议直播 / 推流 / 回看 / 会议 API 集成、文件预览 / 在线转换、短信注册、用户批量导入、专家库批量导入、操作审计日志、更细粒度权限矩阵、AI 配置管理和 AI 调用日志仍未实现。
...
5. 演示版基础闭环已完成：服务器试跑和甲方演示基本就绪；人工演示 / 联调以 `reviewx_dev` 为准，`reviewx_test` 只用于自动化测试；当前库无 pending 申诉和 legacy `active` 材料样本，legacy `active` 仍为无样本未验证。
6. 后续按实际需求进入部署维护、小范围演示打磨或暂缓能力专项开发：短信验证码登录前后端已完成；短信验证码找回密码前后端已完成；腾讯会议直播 / 推流 / 回看 / 会议 API 集成、文件预览 / 在线转换、审计日志、批量导入增强、权限矩阵配置、AI 配置管理和 AI 调用日志仍暂缓；AI 辅助生成合议草稿已接入，最终确认仍人工复核。
...
- 当前阶段建议：演示版基础闭环已完成；AI 辅助生成合议草稿已接入百炼兼容 Chat，后续按实际需求进入部署维护、小范围演示打磨或暂缓能力专项开发，不要把 AI 配置管理、调用日志、流式输出等暂缓能力写成已完成能力。
- 短信验证码登录前后端已完成；短信验证码找回密码前后端已完成；短信注册仍未实现。
...
- AI 合议草稿只允许写入 draft 字段，不得自动确认最终合议，不得自动填入最终确认表单，不得修改 `Project.finalLevel`，不得读取 OSS 材料内容或文件预览；AI 失败时 `rule_based` fallback 保证流程不中断。
...
- AI 辅助生成合议草稿已接入百炼 OpenAI-compatible Chat；AI 配置管理页面、调用日志、流式输出、多模型管理和材料全文入 prompt 仍暂缓，`rule_based` 保留为 fallback。
- 文件预览 / 在线转换暂缓；当前只提供短期下载 URL。
- 操作审计日志暂缓，除非后续监管或合规需求明确。
- 短信验证码找回密码前后端已完成，不再作为暂缓项；用户批量导入、专家库批量导入、权限矩阵配置和“短信验证码找回密码”以外的账号安全增强仍暂缓；短信验证码登录前后端已完成，用户自助改密已完成；短信注册仍未实现。
...
- AI 辅助合议草稿已接入百炼兼容 Chat；生产需配置 `LLM_PROVIDER=bailian`、`BAILIAN_API_KEY`、`BAILIAN_BASE_URL`、`BAILIAN_MODEL`，未配置或调用失败会 fallback 到 `rule_based`。
- 文件存储生产依赖 OSS 配置，development / test 默认 fake。
- 短信验证码登录和短信验证码找回密码后端均依赖 `SMS_AUTH_PROVIDER=aliyun` 与 `ALIYUN_SMS_*` 配置；自动化测试固定走 stub，不调用真实阿里云短信 API，不保存或返回验证码；找回密码成功后不自动登录并清理已有 sessions。
- 用户数据、项目数据、评审数据目前还未做细粒度操作审计。
- 后续增强建议继续分阶段推进，不要把暂缓能力一次性混入基础闭环维护。
...
- 当前 `reviewx_dev` 无 pending 申诉和 legacy `active` 材料样本；`hasPendingAppeal=true` 筛选可用但可能为空结果。
- 专家评分退回后重提链路已在第十阶段补丁一通过 HTTP 写入验证；当前实现重提会清空 returned 留痕，因此第十一阶段只读预检无法从当前持久数据反推该历史。
