...
本路线图描述的是截至“ReviewX 前端第五阶段：专家工作台与评分”完成，并完成“ReviewX 小修：AdminShell 增加返回工作台入口”和“ReviewX 小修：普通字典类型筛选固定化并默认选中项目状态”后的项目状态。
...
- 已修复：
  - 普通字典 `dictType` 中文化
  - 普通字典页类型筛选固定为项目状态和材料类型，默认选中项目状态，不再提供全部浏览或自定义类型录入
  - `code` 显示为“编码”
  - 树形字典缩进展示
...
阶段外小修：ReviewX 小修：普通字典类型筛选固定化并默认选中项目状态（已完成）
   ...
   - 字典类型筛选仅保留 `project_status` 和 `material_type`
...
- 管理员普通字典页已固定为平台内置普通字典类型维护入口，默认加载项目状态，仅允许在项目状态和材料类型之间切换；不再提供全部浏览、自定义类型筛选或自定义 `dictType` 输入。
