...
| 项目负责人材料删除 | `{ deleted, alreadyDeleted?, deletionLogId? }` | 不返回 OSS AccessKey 或文件内容 | 仅 `draft/legacy active` 可删除；先删 storage object，成功后写 `project_material_deletion_logs` 并物理删除 `project_materials` 主记录；`submitted` 返回 `409`；不再返回 `alreadyDeleted=true` |
